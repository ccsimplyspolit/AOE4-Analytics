...
export function VideoPlayer({
  ...
  return (
    <div className={`overflow-hidden rounded-md border border-border/70 bg-background/40 ${className}`}>
      {playing ? (
        ...
        <button
          ...
          className="group flex aspect-video w-full flex-col items-center justify-center gap-2 bg-gradient-to-br from-primary/20 via-secondary to-background px-5 text-center transition-colors hover:from-primary/30"
        >
          <span className="rounded-full bg-primary p-3 text-primary-foreground shadow-lg transition-transform group-hover:scale-105">
      ...
      >
        {video.provider === 'twitch' ? 'Twitch' : 'YouTube'} <ExternalLink className="h-3 w-3" />
      </a>
    </div>
  )
}
